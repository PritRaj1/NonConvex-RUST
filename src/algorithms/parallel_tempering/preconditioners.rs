...
where
    ...
    D: Dim + Send + Sync,
    DefaultAllocator:
        Allocator<D> + Allocator<N, D> + Allocator<D, D> + Allocator<U1, D> + Allocator<N>,
{
    fn compute_covariance(
        &self,
        population: &OMatrix<T, N, D>,
        ...
        if valid_individuals.is_empty() {
            return OMatrix::<T, D, D>::identity_generic(D::from_usize(dim), D::from_usize(dim))
                * self.regularization;
        }
        ...
        let mut cov = OMatrix::<T, D, D>::zeros_generic(D::from_usize(dim), D::from_usize(dim));
    ...
    OMatrix<T, D, D>: Send + Sync,
{
    fn compute_covariance(
        ...
        let elite_count = ((valid_data.len() as f64 * self.elite_fraction.to_f64().unwrap()).ceil()
            as usize)
            .max(1);
...
where
    ...
    DefaultAllocator: Allocator<D> + Allocator<D, D>,
{
    pub fn new(initial_regularization: T, adaptation_rate: T, target_acceptance_rate: T) -> Self {
        Self {
    ...
    pub fn update_regularization(&mut self, acceptance_rate: T) {
        ...
        self.regularization = RealField::min(
            RealField::max(self.regularization, self.min_regularization),
            self.max_regularization,
        );
    }
}
