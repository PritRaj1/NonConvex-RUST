    ...
    DefaultAllocator: Allocator<D> + Allocator<N, D> + Allocator<N>,
{
    ...
    acquisition: AcquisitionFunctionPtr<T, D>,
    best_observations: Vec<(OVector<T, D>, T)>,
    worst_observations: Vec<(OVector<T, D>, T)>,
    ...
    adaptive_noise_scale: T,
    noise_scale_history: Vec<T>,
    bounds_cached: bool,
    cached_lower_bounds: OVector<T, D>,
    cached_upper_bounds: OVector<T, D>,
    stagnation_window: usize,
    meta_optimization_history: Vec<(T, T)>,
    meta_optimization_iter: usize,
    kde_refit_frequency: usize,
    observations_changed: bool,
    last_kde_fit_iter: usize,
