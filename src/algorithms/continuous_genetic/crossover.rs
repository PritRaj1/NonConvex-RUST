...
impl Random {
    pub fn new(crossover_prob: f64, population_size: usize, seed: u64) -> Self {
        Self {
            crossover_prob,
            population_size,
            seed,
...
impl SimulatedBinary {
    pub fn new(crossover_prob: f64, eta_c: f64, population_size: usize, seed: u64) -> Self {
        Self {
            crossover_prob,
            eta_c,
            population_size,
                    ...
                    if rng.random::<f64>() <= crossover_prob && num_parents >= 2 {
                        ...
                        for k in 0..parents.ncols() {
                            ...
                            if (y1 - y2).abs() > T::cast(1e-14) {
                                ...
                                let c1 =
                                    half * ((T::one() + beta_t) * y1 + (T::one() - beta_t) * y2);
                                let c2 =
                                    half * ((T::one() - beta_t) * y1 + (T::one() + beta_t) * y2);
